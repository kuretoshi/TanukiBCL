...
const Avatar: React.FC<AvatarProps> = function ({
	...
	const canvas = simpleAvatar ? (
		<LiteAvatar
			colorPalette={colorPalette ?? playerColors[displayColor]}
			player={player}
			size={size}
			talking={talking}
	...
	if (socketConfig) {
		return (
			<Tooltip
				title={<PlayerConfigTooltip name={displayName} config={socketConfig} onChange={onConfigChange} />}
				leaveDelay={300}
				arrow
				placement="top"
			>
				<Box sx={classes.relative}>
					<Box sx={{ visibility: hideAvatar ? 'hidden' : 'visible' }}>{canvas}</Box>
					{icon}
				</Box>
			</Tooltip>
		);
