	...
	const players = (state.players ?? [])
		.map((player) =>
			[
				...
				player.skinId,
				player.visorId,
				player.nosLobbyColor,
				player.nosPlayer?.colorR,
				player.nosPlayer?.colorG,
				player.nosPlayer?.colorB,
				player.shiftedColor,
				player.inVent ? 1 : 0,
				player.isDead ? 1 : 0,
				player.isLocal ? 1 : 0,
		...
		.join(';');
	return `${state.mixupSabotaged}|${state.camouflaged}|${state.gameState}|${state.oldMeetingHud ? 1 : 0}|${state.mod}|${players}`;
}
