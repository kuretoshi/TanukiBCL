...
相手の声が聞こえるかどうかは、相手 peer から音声ストリームを受信できていることを前提に、`calculateVoiceAudio()` の最終ゲインが 0 より大きいかで決まる。

主な無音条件:

- 相手が `disconnected` または `isDummy`
- ゲーム状態が `MENU` または `UNKNOWN`
- タスク中に `meetingGhostOnly` が有効
- タスク中に通信サボタージュ中、かつ自分が生存クルー側
- 相手がベント中で、ベント内インポスター発話許可条件を満たさない
- タスク中に相手が死亡済みで、自分が生存者かつ `haunting` / `thirdPartyHaunting` の対象外
- `deadOnly` が有効で、自分または相手のどちらかが生存している
- 距離判定で自分またはカメラ位置から相手までが最大距離を超える
- `wallsBlockAudio` が有効で、自分が生存中、かつ自分と相手の間に壁または閉鎖ドアがある
- 会議中に自分が生存、相手が死亡済み

最大距離は通常 `lobbySettings.maxDistance` を使う。`visionHearing` が有効な場合、生存クルー側は `gameState.lightRadius + 0.5` を使い、インポスターは `maxDistance` を維持する。算出値が 0.6 以下の場合は 1 に補正する。

`hearThroughCameras` が有効でタスク中にカメラを見ている場合、自分から相手ではなく、現在カメラ位置から相手までの距離で再判定する。Skeld のカメラでは相手に最も近いカメラ位置を使う。

#### 10.3.2 壁・ベント・カメラによる音声加工

壁衝突は最終的に無音化する。ベント中またはカメラ経由で聞こえる場合は、音量を下げたうえでマッフルを適用する。

| 状況 | 処理 |
| --- | --- |
| 自分または相手がベント中 | 最大距離 0.8 相当、マッフル適用、通常ゲインは 0.5 |
| カメラ経由 | 最大距離 3 相当、マッフル適用、通常ゲインは 0.8 |
| インポスター無線 | 距離判定をスキップし、ハイパス系の無線音質を適用 |
| Airship スポーン選択中フォールバック | 距離判定をスキップし、中央定位にする |
...
ボイスエフェクトはタスク中のみ判定される。適用条件は以下の通り。

- ロビー設定 `voiceEffectEnabled` が `false` ではない
- 自分と相手がどちらも生存している
- マッフル系の音声加工が優先されていない
- 見た目変化エフェクトの場合、ゲーム中の誰かに `appearanceName != name` の見た目変化があり、対象相手にも見た目変化がある
- 見た目変化エフェクトの場合、ユーザー設定 `voiceEffectStrength` が 0 より大きい
- サイズエフェクトの場合、相手の `sizeScale` が 1.0 から 0.12 以上ずれている

見た目変化エフェクトとサイズエフェクトが同時に成立する場合は、見た目変化エフェクトを優先する。サイズエフェクトでは、相手が大きい場合は低め、小さい場合は高めのフォルマント/ピッチ方向に加工する。
