...
for (const lite of [false, true]) for (const mod of ['SUPER_NEW_ROLES', 'NoS', 'NONE']) {
  ...
  for (const label of labels) assert.equal(markup.includes(label), mod === 'SUPER_NEW_ROLES', label);
  assert.equal(markup.includes(translations.settings.lobbysettings.nos_neutral_killer_haunting), mod === 'NoS');
